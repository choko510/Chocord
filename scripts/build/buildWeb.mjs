...
async function buildExtension(target, files) {
    const entries = {
        "dist/Chocord.js": await readFile("dist/browser/extension.js"),
        "dist/Chocord.css": await readFile("dist/browser/extension.css"),
...
const appendCssRuntime = readFile("dist/Chocord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Chocord.user.js", cssRuntime);
});
