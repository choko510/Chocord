...
const TrustedRolesIds = [
    ...
    EQUICORD_TEAM, // Chocord Team
    DONOR_ROLE_ID, // Chocord Donor
    CONTRIB_ROLE_ID, // Chocord Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Chocord/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Chocord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Chocord/Chocord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Chocord DevBuild": !IS_STANDALONE,
...
export default definePlugin({
    ...
    commands: [
        {
            name: "equicord-debug",
            description: "Send Chocord debug info",
            // @ts-ignore
        ...
        },
        {
            ...
            description: "Send Chocord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isEquicordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Paragraph>You are using an outdated version of Chocord! Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using an externally updated Chocord version, the ability to help you here may be limited.</Paragraph>
                        <Paragraph className={Margins.top8}>
                            Please join the <Link href="https://equicord.org/discord">Chocord Server</Link> for support,
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of Chocord, which we do not provide support for!</Paragraph>

                        <Paragraph className={Margins.top8}>
                            We only provide support for <Link href="https://github.com/Chocord/Chocord">official builds</Link>.
                            Either <Link href="https://github.com/Chocord/Equilotl">switch to an official build</Link> or figure your issue out yourself.
                        </Paragraph>
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Chocord & Vencord plugin developers for support!
                <br />
