...
function useSizing(collapsed, tableRef, modal, content, lang, showPreview) {
  ...
  return (0, import_react4.useLayoutEffect)(() => {
    if (ref.current = !1, modal)
      ...
      let { height: height2 } = getComputedStyle(tableRef.current.parentElement, "::-webkit-scrollbar");
      let scrollerHeight = parseFloat(height2);
      if (isNaN(scrollerHeight)) scrollerHeight = 0;
...
var import_react8, ModalRoot, Spinner, foundToolTip, Tooltip, Switch, Popout, ErrorBoundary, init_components = __esm({
  "src/components/index.tsx"() {
    ...
    init_icon();
    init_settingsItem();
    ModalRoot = BdApi.Webpack.getByStrings("reducedMotion:", "transitionState:", "small", "dialog", { searchExports: !0 }), Spinner = BdApi.Webpack.getModule((m) => m.Type?.PULSING_ELLIPSIS, { searchExports: !0 }), foundToolTip = BdApi.Webpack.getModule((m) => m.prototype?.setDomElement && m.prototype.render.toString().includes("renderTooltip()"), { searchExports: !0 }), Tooltip = foundToolTip || BdApi.Components.Tooltip, Switch = BdApi.Webpack.getModule(BdApi.Webpack.Filters.byStrings("focusProps:", ",tooltipNote:"), { searchExports: !0 }), Popout = BdApi.Webpack.getModule((m) => m.prototype?.render?.toString().includes("shouldShowPopout"), { searchExports: !0 });
    Spinner || (Spinner = (props) => BdApi.React.createElement("span", { className: "ECBlock-spinnerFallback", "data-type": String(props?.type ?? "") }), Spinner.Type = {
      WANDERING_CUBES: "WANDERING_CUBES",
      PULSING_ELLIPSIS: "PULSING_ELLIPSIS"
    });
    Tooltip || (Tooltip = ({ children }) => typeof children == "function" ? children({}) : children);
    Switch || (Switch = ({ value, onChange, children }) => BdApi.React.createElement("label", { className: "ECBlock-switchFallback" }, BdApi.React.createElement("input", { type: "checkbox", checked: !!value, onChange: (event) => onChange?.(event.currentTarget.checked) }), BdApi.React.createElement("span", null, children)));
    Popout || (Popout = ({ children }) => typeof children == "function" ? children({
      onClick: () => {
      },
      onMouseEnter: () => {
      },
...
var import_react9, import_highlight2, SearchPopout, SearchItem, languageSelector, LANGUAGES, changeLang_default, init_changeLang = __esm({
  "src/codeblock/changeLang.tsx"() {
    ...
    init_util();
    SearchPopout = BdApi.Webpack.getModule((m) => m.toString?.().includes('4o4z3e"]'), { searchExports: !0 }), SearchItem = BdApi.Webpack.getModule((m) => m.Checkbox && m.Checkmark, { searchExports: !0 }), SearchPopout || (SearchPopout = ({ children }) => BdApi.React.createElement("div", null, typeof children == "function" ? children("") : children)), SearchItem || (SearchItem = ({ children }) => BdApi.React.createElement("div", null, children)), SearchItem.Label || (SearchItem.Label = ({ children }) => BdApi.React.createElement("span", null, children)), { languageSelector } = BdApi.Webpack.getModule((m) => m.languageSelector && m.fileName), LANGUAGES = import_highlight2.default.listLanguages().map((name) => {
...
var BdApi2, codeBlock, MessageAttachment, messageListItem, ECBlocks, src_default, init_src = __esm({
  "src/index.tsx"() {
    ...
    init_components();
    let parserModule;
    BdApi2 = new window.BdApi("ECBlocks"), parserModule = BdApi2.Webpack.getModule((m) => m.parse && m.parseTopic), codeBlock = parserModule?.defaultRules?.codeBlock, MessageAttachment = BdApi2.Webpack.getModule((m) => m.defaultProps?.renderEmbeds, { searchExports: !0 }), { messageListItem } = BdApi2.Webpack.getModule((m) => m.messageListItem), ECBlocks = class {
      forceUpdateMessages() {
      ...
      start() {
        codeBlock && typeof codeBlock.react == "function" && BdApi2.Patcher.after(codeBlock, "react", (that, [props], res) => BdApi2.React.createElement(ErrorBoundary, { fallback: res }, BdApi2.React.createElement(codeblock_default, { ...props, modal: !1, fileName: () => `codeblock-${Date.now()}.${props.lang || "txt"}` }))), MessageAttachment?.prototype && typeof MessageAttachment.prototype.renderAttachments == "function" && BdApi2.Patcher.after(MessageAttachment.prototype, "renderAttachments", (that, props, res) => {
          if (res)
        ...
        }), BdApi2.DOM.addStyle("EnhanceCodeBlocks", css_default), this.forceUpdateMessages();
      }
      stop() {
        BdApi2.Patcher.unpatchAll(), BdApi2.DOM.removeStyle("EnhanceCodeBlocks"), this.forceUpdateMessages();
      }
      getSettingsPanel() {
