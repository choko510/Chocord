...
const resolveStyleDescriptors = (module) => {
    if (!module || (typeof module !== "object" && typeof module !== "function")) return [];
    try {
        return Object.entries(Object.getOwnPropertyDescriptors(module));
    } catch (error) {
        Logger.warn("Failed to extract style descriptors from a module.", error);
        return [];
    }
};
const styleModules = [
...
const styles = Object.defineProperties({},
    Object.fromEntries(
        styleModules.flatMap(resolveStyleDescriptors)
    )
);
...
function Popout({
    ...
    React.useInsertionEffect(() => {
        ...
        const vencordRoot = document.querySelector("vencord-root");
        if (!root || !vencordRoot) return;
        root.querySelector("vencord-root")?.remove();
        root.appendChild(vencordRoot.cloneNode(true));
    }, []);
...
function PopoutContent({
    ...
    return React.createElement(
        ...
        React.createElement(IdleDetector, {
            ...
            ), FullScreenButton ? React.createElement(
                FullScreenButton, {
                    className: styles.rightTrayIcon,
